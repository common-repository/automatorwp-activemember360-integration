...
final class AutomatorWP_ActiveMember360_Integration {
    ...
    private function constants() {
        // Plugin version
        define( 'AUTOMATORWP_ACTIVEMEMBER360_VER', '1.0.1' );
    ...
    public function admin_notices() {
        ...
        if ( ! $this->meets_requirements() && ! defined( 'AUTOMATORWP_ADMIN_NOTICES' ) ) : ?>
            ...
            <div id="message" class="notice notice-error is-dismissible">
                <p>
                    <?php printf(
                        __( 'AutomatorWP - ActiveMember360 requires %s and %s in order to work. Please install and activate them.', 'automatorwp-activemember360-integration' ),
                        '<a href="https://wordpress.org/plugins/automatorwp/" target="_blank">AutomatorWP</a>',
                        '<a href="https://activemember360.com/" target="_blank">ActiveMember360</a>'
                    ); ?>
                </p>
            </div>
        ...
        <?php elseif ( $this->pro_installed() && ! defined( 'AUTOMATORWP_ADMIN_NOTICES' ) ) : ?>
            ...
            <div id="message" class="notice notice-error is-dismissible">
                <p>
                    <?php echo __( 'You can uninstall AutomatorWP - ActiveMember360 Integration because you already have the pro version installed and includes all the features of the free version.', 'automatorwp-activemember360-integration' ); ?>
                </p>
            </div>
    ...
    public function load_textdomain() {
        ...
        $lang_dir = AUTOMATORWP_ACTIVEMEMBER360_DIR . '/languages/';
        $lang_dir = apply_filters( 'automatorwp_activemember360_languages_directory', $lang_dir );

        // Traditional WordPress plugin locale filter
        $locale = apply_filters( 'plugin_locale', get_locale(), 'automatorwp-activemember360-integration' );
        $mofile = sprintf( '%1$s-%2$s.mo', 'automatorwp-activemember360-integration', $locale );

        // Setup paths to current locale file
        $mofile_local   = $lang_dir . $mofile;
        $mofile_global  = WP_LANG_DIR . '/automatorwp-activemember360-integration/' . $mofile;

        if( file_exists( $mofile_global ) ) {
            // Look in global /wp-content/languages/automatorwp-activemember360-integration/ folder
            load_textdomain( 'automatorwp-activemember360-integration', $mofile_global );
        } elseif( file_exists( $mofile_local ) ) {
            // Look in local /wp-content/plugins/automatorwp-activemember360-integration/languages/ folder
            load_textdomain( 'automatorwp-activemember360-integration', $mofile_local );
        } else {
            // Load the default language files
            load_plugin_textdomain( 'automatorwp-activemember360-integration', false, $lang_dir );
        }
...
add_action( 'plugins_loaded', 'AutomatorWP_ActiveMember360_Integration' );
